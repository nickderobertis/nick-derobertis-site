...
| `resolveTsconfigAliases` is not reintroduced and `tsconfig.base.json` gains no project `paths` | `grep -rn resolveTsconfigAliases` over the tree matches only this document's history paragraph, and `grep -n paths tsconfig.base.json` matches nothing. |
