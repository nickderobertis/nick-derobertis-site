...
| Correctness does not depend on key order | The ordered map that decided it is gone: an `exports` map is keyed by exact subpath, so no key in it can claim the path below another and there is no order for a resolution to depend on. What is still an ordered map is `remotes`, and no host in this workspace declares two remotes where one begins with the other; the one overlap that does exist — a remote against a specifier a package publishes — is decided by mechanism rather than order, because an alias is applied before manifest resolution is reached, which the probe's "answers with the remote where a package publishes the same specifier" resolves for real. |
...
| Proven end to end through a configuration `defineWorkspaceTestConfig` produced | `scripts/workspace/subpath-resolution-probe/` runs under a config that harness produced, imports every subject for real, and is driven as a real Vitest subprocess by `subpath-resolution.spec.ts` › "answers every longer specifier the same way under the shared test harness". |
