...
| Configuration is simpler | **Disproved for the partial slice** | 34 lines of rspack build config were removed and 163 lines of Rsbuild config were added: **129 more build-config lines**. The increase includes the runtime validation and single-source derivation required after losing the Nx executor and rspack-specific shared helper. The two Nx project files changed by +10/-14 lines. Zero scripts and zero script lines could be deleted. | Even before adding an SSG integration, local config is substantially longer and the Nx boundary less structured. |
