...
- Coverage is 95% on lines, functions, branches, and statements, for app UI as well as library code. One exemption exists, and it is the only one: the `tooling-*` projects, because their subjects run as real subprocesses v8 cannot instrument — every workspace script, `just` recipe, and hook must instead be driven by a spec or record why it cannot be. Every other project owes a `test` target and that floor, declared as thresholds in its own component config rather than inherited from the shared harness; `scripts/workspace/structure-contract.spec.ts` derives the owed set from this sentence rather than from what each project happens to declare. That includes `design-system`, which was the second exemption while it published `theme.css` and nothing else: it now publishes the shared page shell, section heading, card, action link, pane state and skeleton primitives every app composes its pages from, so it declares a `test` target and meets the floor over them, while the stylesheet half stays verified in the browser by `site.spec.ts`, by home-cards' design-system-primitives journey through both render paths, by each remote's standalone design-system journey, and by every app's screencomp capture.
