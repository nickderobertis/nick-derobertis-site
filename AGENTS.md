...
2. Federation ownership: all 12 remotes render without failed assets through both standalone and host-composed boundaries. The deploy topology is covered separately from the local one: composing a content-store-shaped fragment root into a fresh output directory, serving that directory, and loading every route and standalone remote from it produces no failed request.
