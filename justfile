...
@build-app app:
    ...
    app="$1"; lanes=$({{node_typestrip}} scripts/publishable-apps.mjs --all) || { echo "build-app: the publish lanes could not be resolved; fix the error above and rerun just build-app <app>" >&2; exit 1; }; printf '%s' "$lanes" | grep -qF "\"$app\"" || { echo "build-app: app must name a publish lane, one of $lanes. A buildable library owns no content-store subtree, so pass one name from just publish-lanes and rerun just build-app <app>" >&2; exit 2; }; log=$(mktemp); trap 'rm -f "$log"' EXIT; CI=1 pnpm exec nx run "$app:build" >"$log" 2>&1 || { cat "$log" >&2; echo "build-app: building $app failed; fix the errors above and rerun just build-app $app" >&2; exit 1; }
...
# scripts/publish-fragment.mjs validates before it touches git.
@publish-fragment:
    ...
    {{node_typestrip}} scripts/publish-fragment.mjs
