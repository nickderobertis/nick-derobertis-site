...
check-all: lint-workflows
    ...
    log=$(mktemp); trap 'rm -f "$log"' EXIT; pnpm exec biome check --error-on-warnings . >"$log" 2>&1 && CI=1 pnpm exec nx run-many -t lint --all --parallel=3 --args="--error-on-warnings" >>"$log" 2>&1 && CI=1 pnpm exec nx run-many -t typecheck,test,build,prerender --all --parallel=3 >>"$log" 2>&1 && CI=1 pnpm exec nx run-many -t e2e,screenshot --all --parallel=3 >>"$log" 2>&1 && CI=1 pnpm exec nx run shell:e2e >>"$log" 2>&1 || { cat "$log" >&2; echo "check-all: quality gate failed; fix warnings and errors above, then rerun just check-all" >&2; exit 1; }
...
# Rewrite the canonical remote registry from the remotes the project graph
...
# llmlint: ignore-block[changed_behavior_has_e2e] This developer CLI has no browser interface; whether an edit recomposes the artifact is a graph fact with nothing for a visitor to observe. affected-build-projects.spec.ts drives its real `just` subprocess through both selections it reports and its rejected-input path, and site.spec.ts drives the artifact the selection governs in a real browser.
