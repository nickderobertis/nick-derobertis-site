...
@lint-llm-diff base="origin/master" *files:
    base="$1"; [[ "$base" != -* ]] && git rev-list -1 "$base" -- >/dev/null 2>&1 || { echo "lint-llm-diff: base must be a git revision to diff against, such as origin/master, HEAD~1, or a range; fetch the missing ref, then rerun just lint-llm-diff <base>" >&2; exit 2; }; llmlint_args=(--diff --diff-base "$base"); for argument in "${@:2}"; do if [[ "$argument" != "--skip-nx-cache" ]]; then [[ "$argument" != -* && "$argument" != *..* && -e "$argument" ]] || { echo "lint-llm-diff: every file after the base must be an existing workspace path, because llmlint reports a clean run for a path it cannot match; correct or drop \"$argument\", then rerun just lint-llm-diff $base <files>" >&2; exit 2; }; llmlint_args+=("$argument"); fi; done; llmlint "${llmlint_args[@]}" || { echo "lint-llm-diff: the LLM judge reported the findings above; fix each one, or justify it with a narrow ignore directive at its site, then rerun just lint-llm-diff" >&2; exit 1; }
