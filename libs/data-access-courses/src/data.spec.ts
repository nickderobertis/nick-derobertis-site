 ...
 * import. Each case loads `./data` fresh — once over the committed courses
 ...
 */
afterEach(() => {
  vi.doUnmock("@site/data-access-core/domains/courses");
  vi.resetModules();
});

describe("the committed courses slice", () => {
  it("exports the committed domain once the schema has accepted it", async () => {
    const { courses } = await import("./data");

    expect(courses.length).toBeGreaterThan(0);
    expect(courses.every((course) => typeof course.title === "string")).toBe(
      true,
    );
  });

  it("refuses at import when the slice does not satisfy the schema", async () => {
    // A course record carrying an id but not the title the schema
    // requires beside it.
    vi.resetModules();
    vi.doMock("@site/data-access-core/domains/courses", () => ({
      coursesArtifact: [{ id: "financial-modeling" }],
    }));

    await expect(import("./data")).rejects.toThrow(
      "CV courses domain failed schema validation",
    );
  });
});
