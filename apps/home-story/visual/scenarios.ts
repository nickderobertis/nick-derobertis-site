...
// skeleton drops its portrait and copy from two columns to one under 48rem, and
// the empty and error panels are sized from the viewport rather than from their
// content. A visitor who previews any of those on a tablet or a phone therefore
// sees a layout no shot covers, so each state is captured at those widths too.
