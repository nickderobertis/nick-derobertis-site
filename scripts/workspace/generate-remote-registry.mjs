...
// The failures here are corrected in different places — a misspelled flag, a
// graph Nx would not resolve, a declaration the registry module refuses, a
// committed registry that no longer reads back — so this names none of them:
// every message above names the file at fault, and the ones whose remedy is
// particular to them name that too. Naming a declaration here sent a
// contributor past the file that was actually broken. The second line is what
// holds for every failure, including one this CLI never anticipated, whose
// message names nothing to act on by itself.
process.on("uncaughtException", (error) => {
  console.error(
    `generate-remote-registry: ${error instanceof Error ? error.message : String(error)}
Fix what that names, or run just bootstrap to restore the workspace's pinned tooling if it names nothing you can act on, then rerun just generate-remote-registry.`,
  );
  process.exit(1);
});
...
function committedRegistry() {
  try {
    ...
    throw new Error(
      `${registryPath} could not be read: ${error instanceof Error ? error.message : String(error)}. Restore that path from git, or delete it to have this run write it.`,
    );
  }
}
...
function committedOrder(committed) {
  ...
  try {
    ...
    throw new Error(
      `${registryPath} is not readable as JSON: ${error instanceof Error ? error.message : String(error)}. Restore it with git checkout ${registryPath}.`,
    );
  }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed))
    throw new Error(
      `${registryPath} is not a JSON object mapping each remote's project name to its federation alias. Restore it with git checkout ${registryPath}.`,
    );
  ...
  for (const [name, alias] of Object.entries(parsed))
    if (
      ...
      throw new Error(
        `${registryPath} maps ${JSON.stringify(name)} to ${JSON.stringify(alias)}, which is not a remote's project name and the Module Federation container it publishes under. Restore it with git checkout ${registryPath}.`,
      );
...
if (unrecognized.length > 0)
  throw new Error(
    `takes only --check, which reports whether ${registryPath} still agrees with the project graph, and not ${unrecognized.map((argument) => JSON.stringify(argument)).join(", ")}; with no argument it rewrites that file. Correct the argument.`,
  );
